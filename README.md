...
在 Hanako 设置中启用插件，让 Agent 执行 `hanako-ui-beautify_apply`，重启生效。
...
| 工具 | 用途 |
|---|---|
| `hanako-ui-beautify_status` | 诊断：已应用状态 / 策略可用性 / 权限 / 签名 |
...
| `strategy` | `auto` | 策略：`auto` / `runtime` / `asar` |
...
Windows 上 `C:\Program Files\Hanako` 默认需管理员权限；自 v0.3.0 起，支持运行时注入的版本不再需要提权。详细步骤见 [`INSTALL.md`](INSTALL.md)。
...
if ($bak) { Copy-Item $bak.FullName "$res\app.asar" -Force }
```
