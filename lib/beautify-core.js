...
export function defaultHanakoDir() {
  ...
  const candidates = ["C:\\Program Files\\Hanako", "C:\\Program Files\\HanaAgent"];
...
export function resolvePaths(pluginDir, input = {}) {
  ...
  return {
    ...
    tempDir: path.join(os.tmpdir(), `hanako-ui-beautify-${ts}`),
    tempAsarPath: path.join(os.tmpdir(), `hanako-ui-beautify-${ts}.asar`),
...
export async function restoreBeautify(pluginDir, input = {}) {
  ...
  try {
    asar.listPackage(paths.backupPath);
  } catch (verifyErr) {
