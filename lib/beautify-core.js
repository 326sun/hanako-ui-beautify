...
// backups so the backup dir doesn't grow without bound. The legacy single-file
// backup is left untouched (restore still falls back to it).
function pruneOldBackups(paths, keep = 3) {
  try {
    ...
    for (const stale of hashed.slice(keep)) {
...
export function inspectAsarCompatibility(paths, header) {
  ...
  if (process.platform === "darwin") {
    const bundleRoot = path.resolve(paths.hanakoDir, "..");
    const codeResources = path.join(bundleRoot, "_CodeSignature", "CodeResources");
...
export async function applyBeautify(pluginDir, input = {}) {
  ...
  return await withAsarLock(paths, async () => {
    ...
    // Apply succeeded and the backup we used is the newest — safe to prune old ones.
