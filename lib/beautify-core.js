...
// `options.diagnostics` (default true) controls the expensive read-only probes
// — full-CSS selector scan and renderer health — that extract many files from
// the asar. Internal callers that only need apply/marker/write state (e.g. the
// `before` snapshot inside applyBeautify) pass `false` to skip them.
export function getStatus(pluginDir, input = {}, options = {}) {
  const { diagnostics = true } = options;
...
export async function applyBeautify(pluginDir, input = {}) {
  ...
  return await withAsarLock(paths, async () => {
    const before = await getStatus(pluginDir, input, { diagnostics: false });
    ...
    try {
      ...
      // Read theme + fonts once up front; they are identical for every styles.css
      // we patch, so re-reading them per path is wasted disk I/O.
      const inlineThemeCss = buildInlineThemeCss(paths.themePath);
      const themeCssBuffer = Buffer.from(fs.readFileSync(paths.themePath, "utf-8"), "utf-8");
      const fontBuffers = fontFiles.map((file) => ({ file, data: fs.readFileSync(path.join(paths.fontsDir, file)) }));
      ...
      for (const cssPath of stylesCssPaths) {
        ...
        for (const { file, data } of fontBuffers) {
          additions.push({
