...
export async function createPackageWithOptions(srcDir, destPath, _options = {}) {
  ...
  // 2. Build header tree and compute offsets in one pass.
  //    Offsets MUST be computed before the first JSON.stringify because the
  //    JSON size depends on offset string lengths — using placeholder "0" then
  //    recalculating totalHeaderSize would produce a wrong header size.
  const header = { files: {} };
  ...
  for (const file of fileList) {
    ...
    node.files[name] = { size: file.size, offset: currentOffset.toString() };
    if (file.executable) node.files[name].executable = true;
    currentOffset += BigInt(file.size);
  }

  // 3. Serialize header and build Pickle wrappers.
