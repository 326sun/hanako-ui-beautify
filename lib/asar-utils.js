 ...
 *
 * Strategy:
 *   - General list/extract/create operations delegate to @electron/asar for
 *     format compatibility with all Electron versions.
 *   - Custom `transformPackage` preserves existing ASAR metadata (executable,
 *     unpacked, integrity, unknown keys) while replacing only targeted files.
 *   - The plugin refuses to patch archives carrying Electron ASAR integrity
 *     metadata to avoid bricking startup on newer Hanako builds.
 *
 * ASAR format (Pickle flavour, as used by @electron/asar):
...
// Lazy-load @electron/asar: resolve at first ASAR operation rather than at
// module init time. This keeps plugin activation fast and avoids crashing
// when the dev-slot runtime hasn't provisioned node_modules yet.
let _electronAsar = null;
function getElectronAsar() {
  if (_electronAsar === null) {
    const require = createRequire(import.meta.url);
    _electronAsar = require("@electron/asar");
  }
  return _electronAsar;
}
...
function readHeader(fd) {
  ...
  const expectedHeaderSize = 8 + align4(jsonLen);
  if (totalHeaderSize !== expectedHeaderSize) {
    throw new Error(`ASAR header size mismatch: sizePickle declares ${totalHeaderSize}, but headerPickle computes to ${expectedHeaderSize} bytes`);
  }
  // headerSize = sizePickle(8) + headerPickle
  return { headerJSON, headerSize: 8 + totalHeaderSize };
}
...
export function listPackage(asarPath) {
  getElectronAsar().uncache?.(asarPath);
...
export function extractFile(asarPath, filePath) {
  ...
  getElectronAsar().uncache?.(asarPath);
  try {
    return getElectronAsar().extractFile(asarPath, normalized);
  } catch (err) {
    const fd = fs.openSync(asarPath, "r");
    try {
      const { headerJSON } = readHeader(fd);
...
export function extractAll(asarPath, destDir) {
  getElectronAsar().uncache?.(asarPath);
  getElectronAsar().extractAll(asarPath, destDir);
}
...
export async function createPackageWithOptions(srcDir, destPath, _options = {}) {
  await getElectronAsar().createPackageWithOptions(srcDir, destPath, _options || {});
  getElectronAsar().uncache?.(destPath);
}

/**
 * Serialize an ASAR header tree plus an ordered list of file-content "parts"
 * into a Pickle-format archive. Each part is either an in-memory buffer
 * (`{ buffer }`, used for replaced/added files) or a copy instruction
 * (`{ srcFd, srcOffset, size }`, used for unchanged files). Copy parts are
 * streamed in fixed-size chunks so the whole archive is never held in memory —
 * peak usage is one chunk plus the (small) replacement buffers, instead of a
 * full second copy of app.asar. Part order MUST match the offset order assigned
 * in the header (i.e. the order leaves are visited when walking).
 */
const COPY_CHUNK_BYTES = 1 << 20; // 1 MiB

// fs.writeSync is a single write() syscall and may write fewer bytes than
// requested. For a tool whose whole job is not to corrupt app.asar, loop until
// the full buffer is on disk.
function writeAllSync(fd, buffer, offset = 0, length = buffer.length - offset) {
  let written = 0;
...
function serializeAndWrite(destPath, header, parts) {
  ...
  let chunk = null;
  try {
    writeAllSync(fd, sizePickleBuf);
    writeAllSync(fd, headerPickle);
    for (const part of parts) {
      if (part.buffer) {
        writeAllSync(fd, part.buffer);
        continue;
      }
      // Stream-copy `size` bytes from the source fd at `srcOffset`.
      if (!chunk) chunk = Buffer.allocUnsafe(COPY_CHUNK_BYTES);
      let remaining = part.size;
      let pos = part.srcOffset;
      while (remaining > 0) {
        const want = Math.min(COPY_CHUNK_BYTES, remaining);
        const got = fs.readSync(part.srcFd, chunk, 0, want, pos);
        if (got <= 0) throw new Error(`Short read copying archive bytes at offset ${pos}`);
        writeAllSync(fd, chunk, 0, got);
        pos += got;
        remaining -= got;
      }
    }
...
export function transformPackage(srcAsar, destAsar, mutations = {}) {
  ...
  try {
    ...
    // Build an ordered list of write "parts" without reading unchanged file
    // bytes into memory: replaced/added files carry their buffer, unchanged
    // files carry a copy instruction streamed later from the source fd.
    const parts = [];
    let offset = BigInt(0);
    (function walk(node, prefix) {
      if (!node.files) return;
      for (const [name, child] of Object.entries(node.files)) {
        ...
        if (replacements.has(full)) {
          const data = replacements.get(full);
          parts.push({ buffer: data });
          size = data.length;
        } else if (additionMap.has(full)) {
          const data = additionMap.get(full);
          parts.push({ buffer: data });
          size = data.length;
        } else {
          ...
          size = srcInfo.size;
          parts.push({ srcFd: fd, srcOffset: headerSize + parseInt(srcInfo.offset, 10), size });
        }
        child.size = size;
        if (offset > BigInt(Number.MAX_SAFE_INTEGER)) {
          throw new Error(`ASAR archive exceeds maximum supported size: offset ${offset} > Number.MAX_SAFE_INTEGER`);
        }
    ...
    serializeAndWrite(destAsar, newHeader, parts);
  } finally {
    fs.closeSync(fd);
  }
}
