...
export async function getAdaptiveStatus(ctx, input = {}) {
  ...
  if (status.asarCompatibility?.patchSupported && status.canWrite) status.supportedStrategies.push("asar-transform");
...
export async function applyAdaptiveBeautify(ctx, input = {}) {
  ...
  const status = await getAdaptiveStatus(ctx, input);
  if (!status.asarCompatibility?.patchSupported) {
    return {
      ok: false,
      changed: false,
      strategy: "blocked",
      status,
      message: status.asarCompatibility?.integrityProtected
  ...
  return {
    ...
    status: await getAdaptiveStatus(ctx, input),
  };
}
